...
export default function InterviewSession({
  ...
  const handleEndInterview = async () => {
    ...
    // Save all answers to user_answers table and grade them in the background
    if (user) {
      for (const problem of problems) {
        const problemAnswers = allAnswersRef.current[problem.id] || {};
        const subs = subproblemsByProblem[problem.id] || [];

        for (const [key, answerText] of Object.entries(problemAnswers)) {
